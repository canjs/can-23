...
var dev = require("can-log/dev/dev");
	...
	var getKeyComputeData = function (key, scope, readOptions) {
		...
		lookupValue = function(key, scope, helperOptions, readOptions){
			...
			if (! canReflect.valueHasDependencies( computeData) ) {
	...
	var Call = function(methodExpression, argExpressions, hashes){
		if(hashes && canReflect.size(hashes) !== 0) {
	...
	Helper.prototype.helperAndValue = function(scope, helperOptions){
		...
		if (looksLikeAHelper) {
			...
			if(!helper && typeof context[methodKey] === "function") {
				//!steal-remove-start
				dev.warn('can/view/stache/mustache_core.js: In 3.0, method "' + methodKey + '" will not be called as a helper, but as a method.');
				//!steal-remove-end
		...
		if ( !helper && initialValue === undefined) {
			if(looksLikeAHelper) {
				dev.warn('can/view/stache/mustache_core.js: Unable to find helper "' + methodKey + '".');
			} else {
				dev.warn('can/view/stache/mustache_core.js: Unable to find key or helper "' + methodKey + '".');
			}
		}
		//!steal-remove-end
	...
	Helper.prototype.value = function(scope, helperOptions, nodeList, truthyRenderer, falseyRenderer, stringOnly){
		...
		if (! canReflect.valueHasDependencies( compute ) ) {
			return canReflect.getValue( compute );
		} else {
			return compute;
		}
	};
