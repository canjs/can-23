	...
	function makeTest(name, doc) {
		...
		skip("basic tabs", function () {
			...
			can.Component.extend({
				...
				events: {
					" inserted": function () {
						this.parentVM = can.viewModel(this.element[0].parentNode);
						this.parentVM.addPanel(this.viewModel);
					},
					" removed": function () {
		...
		skip("deferred grid", function () {
			...
			var template = can.stache("<grid deferreddata:from='viewModel.deferredData'>" +
				"{{#each items}}" +
				"<tr>" +
		...
		test("nextprev", function () {
			...
			var template = can.stache("<next-prev paginate:from='paginator'></next-prev>");
		...
		test("page-count", function () {
			...
			var template = can.stache("<page-count page:from='paginator.page'></page-count>");
		...
		test("can.viewModel utility", function() {
			...
			equal(can.viewModel(el).attr("foo"), "bar", "one argument grabs the viewModel object");
		...
		skip("can.viewModel creates one if it doesn't exist", function(){
		...
		skip("content extension stack overflow error", function () {
		...
		skip("@ keeps properties live now", function () {
		...
		test('nested component within an #if is not live bound(#1025)', function() {
			...
			equal( innerHTML(frag.firstChild.firstElementChild), 'Hello world.', 'child component is inserted');
		...
		test('removing bound viewModel properties on destroy #1415', function(){
			...
			can.Component.extend({
				...
				events: {
					destroy: function(){
						this.viewModel.attr('state').attr("product", null);
					}
				}
			});

			var frag = can.stache('<destroyable-component state="{this}"></destroyable-component>')(state);
		...
		test('changing viewModel property rebinds {viewModel.<...>} events (#1529)', 2, function(){
			...
			canDomMutate.flushRecords();
			var vm = can.viewModel(can.$(rebind));
			vm.attr('item.name', 'CDN');
		});
		...
		test("component viewModels are initialized with binding values", function(){
			...
			can.stache("<init-viewmodel prop:from='passedProp'/>")({
				passedProp: "value"
			});
		});
		...
		skip("%root property is available in a viewModel", function () {
			var viewModel = can.Map.extend({});
