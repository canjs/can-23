		...
		if (arguments.length > 4) {
			...
			if(root) {
				if(root.isComputed) {
					...
					var obj = computeData.reads.length ? can.compute.read.read(root, computeData.reads.slice(0, last)).value
						: root;
			...
			if (computeData.root) {
				return can.compute.read.read(computeData.root, computeData.reads, options)
					.value;
			}
